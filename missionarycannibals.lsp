...
; The main entry point for this solver is the function MC-DFS, which is calledkm                      
...
(defun final-state (s)
  (cond ((and (= (first s) 3) (and (= (second s) 3) (equal (third s) NIL))) 'T)
  	(t NIl)
  	)
  )
...
(defun next-state (s m c)
	(cond ((> (+ c m) 2) NIL) ; if missionaries + cannibals > 2 return nil
		((> m 2) NIL) ; if missionaries > 2 return nil
		((> c 2) NIL) ; if cannibals > 2 return nil 
		((> m (first s)) NIL) ; if missionaries to move greater than number on that side in s, return nil
		((> c (second s)) NIL) ; else if cannibals to move greater than number on that side in s, return nil
		((and (> (- (second s) c) (- (first s) m)) (> (- (first s) m) 0)) NIL) ; else if number cannibals > number missionaries on old side (not including 0) once move the guys, return nil
		((and (> (+ (- 3 (second s)) c) (+ (- 3 (first s)) m)) (> (+ (- 3 (first s)) m) 0)) NIL) ; else if number cannibals > number missionaries on new side (not including 0) once move the guys, return nil
		(t (list (list (+ (- 3 (first s)) m) (+ (- 3 (second s)) c) (not (third s))))) ; else, move the missionaries and cannibals
		)
  )
...
(defun succ-fn (s)
	(append (next-state s 0 1) (next-state s 1 1) (next-state s 1 0) (next-state s 2 0) (next-state s 0 2))
  )
...
(defun mult-dfs (states path)
	(let ((x (mc-dfs (first states) path)))
		(cond ((NULL states) nil)
			((and (NULL x) (NULL (rest states))) nil)
			((NULL x) (mult-dfs (rest states) path))
			(t x)))
		)
